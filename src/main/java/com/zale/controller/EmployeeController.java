...
import com.zale.service.EmployeeService;
...
public class EmployeeController {
    ...
    public Msg searchEmpsByVague(@RequestParam("content") String content) {
        ...
        //引入pageHelper分页查询，
        // 在查询之前只需要调用PageHelper.startPage()，传入页码，以及每一页显示的数量PageHelper.startPage(1, 1000000);
        PageHelper.startPage(1, 1000000);
        //分页完之后的查询就是分页查询

        // 模糊查询，姓名中包含content的或者id为content的都会被搜索出来
        List<Employee> employees = employeeService.getEmpByValue(content);
...
//        想要连续显示5页，就加上参数5即可
    ...
    public Msg checkuser(@RequestParam("empName")String empName) {
        ...
        if(!empName.matches(regx)) {
            return Msg.fail().add("va_msg", "username must include 6-16 digit,character or -_，or 2-5 chinese");
        }
        else {
            ...
            if(b) {
               return Msg.success();
                ...
                return Msg.fail().add("va_msg", "Username already exists");
            }
        }
    }
